...
public class AttendanceFragment1 extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rooView=inflater.inflate(R.layout.fragment_attendance1, container, false);

        firebaseFirestore = FirebaseFirestore.getInstance();
        classes = rooView.findViewById(R.id.classes);

        //query
        Query query = firebaseFirestore.collection("lecturercourse/"+currentID+"/courselist");
        ...
        adapter = new FirestoreRecyclerAdapter<TimetableModel, AttendanceFragment1.TimetableViewHolder>(options) {
            @NonNull
            @NotNull
            @Override
            public AttendanceFragment1.TimetableViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
                View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.my_row_course, parent,false);
                return new AttendanceFragment1.TimetableViewHolder(view);
            }

            @Override
            protected void onBindViewHolder(@NonNull @NotNull AttendanceFragment1.TimetableViewHolder holder, int position, @NonNull TimetableModel model) {
                holder.txt1.setText(model.getCoursename());
                holder.itemView.setOnClickListener(new View.OnClickListener(){
                    @Override
                    public void onClick(View v){
                        Intent intent = new Intent(getContext(), ManageAttendanceActivity.class);
